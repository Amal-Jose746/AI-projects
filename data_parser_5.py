...
PDF_DIR = r"C:\Temp\Py_Files_sid\Drawings"  
...
def extract_text_from_pdf(pdf_path):
    ...
    for i, page in enumerate(pages):
        print(f" Running TrOCR on Page {i+1}")
        image = page.convert("RGB")
        pixel_values = processor(images=image, return_tensors="pt").pixel_values
        with torch.no_grad():
            generated_ids = model.generate(pixel_values)
        transcription = processor.batch_decode(generated_ids, skip_special_tokens=True)[0]
        text_blocks.append(transcription.strip())
...
def process_pdf_file(pdf_path):
    try:
        ...
        try:
            ...
            print(" JSON parsing failed. Trying to extract valid block...")
...
def main():
    ...
    if all_results:
        ...
        output_path = os.path.join(PDF_DIR, "All_Drawings_Metadata_by_TrOCR.csv")
